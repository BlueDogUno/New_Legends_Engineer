...
void AUTO::init()
{
    auto_mode = FOREARM_INIT;
    last_auto_mode = auto_mode;
    forearm.auto_control(&auto_mode);
...
{
    // 抓取机构
    if (forearm.motor_status[CAN_SPIN_L_MOTOR] == READY && forearm.motor_status[CAN_SPIN_R_MOTOR] == READY)
        status_flag[MINE_SPIN] = READY;
    else
        status_flag[MINE_SPIN] = WAIT;
    
    if (forearm.motor_status[CAN_FOREARM_YAW_MOTOR] == READY)
        status_flag[MINE_YAW] = READY;
    else
        status_flag[MINE_YAW] = WAIT;
    
    if (forearm.motor_status[CAN_FOREARM_SUCTION_MOTOR] == READY)
