...
public:
  ...
  motor_measure_t forearm_motive_motor[4];
  ...
  CAN_TxHeaderTypeDef forearm_tx_message;
  CAN_TxHeaderTypeDef can_tx_message;

  uint8_t mine_can_send_data[8];
  uint8_t forearm_can_send_data[8];
  ...
  void get_forearm_motor_measure(uint8_t num, uint8_t data[8]);

  void can_cmd_mine_motive_motor(int16_t motor1, int16_t motor2, int16_t motor3, int16_t motor4); //矿仓部分动力电机数据

  void can_cmd_forearm_motive_motor(int16_t motor1, int16_t motor2, int16_t motor3, int16_t motor4); //抓取动力电机数据

  const motor_measure_t *get_mine_motive_motor_measure_point(uint8_t i);

  const motor_measure_t *get_forearm_motive_motor_measure_point(uint8_t i);
