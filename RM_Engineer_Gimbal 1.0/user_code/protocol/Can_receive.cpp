...
void Can_receive::get_forearm_motor_measure(uint8_t num, uint8_t data[8])
{
    forearm_motive_motor[num].last_ecd = forearm_motive_motor[num].ecd;
    forearm_motive_motor[num].ecd = (uint16_t)(data[0] << 8 | data[1]);
    forearm_motive_motor[num].speed_rpm = (uint16_t)(data[2] << 8 | data[3]);
    forearm_motive_motor[num].given_current = (uint16_t)(data[4] << 8 | data[5]);
    forearm_motive_motor[num].temperate = data[6];
    forearm_motive_motor[num].last_total_angle = forearm_motive_motor[num].total_angle;
    //计圈
    if (forearm_motive_motor[num].ecd - forearm_motive_motor[num].last_ecd > 4096)
        forearm_motive_motor[num].round_cnt -- ;
    else if (forearm_motive_motor[num].ecd - forearm_motive_motor[num].last_ecd < -4096)
        forearm_motive_motor[num].round_cnt ++ ;   
    //增量式角度环计算
    forearm_motive_motor[num].total_angle = (forearm_motive_motor[num].round_cnt * 360.0f) / 19.0f  + forearm_motive_motor[num].ecd * 360.0f / 19.0f / 8192.0f;
    forearm_motive_motor[num].angle_err = forearm_motive_motor[num].last_total_angle - forearm_motive_motor[num].total_angle;
}
 ...
 * @retval         none
 */
void Can_receive::can_cmd_forearm_motive_motor(int16_t motor1, int16_t motor2, int16_t motor3, int16_t motor4)
{
    uint32_t send_mail_box;
    forearm_tx_message.StdId = CAN_CATCH_MOTIVE_ALL_ID;
    forearm_tx_message.IDE = CAN_ID_STD;
    forearm_tx_message.RTR = CAN_RTR_DATA;
    forearm_tx_message.DLC = 0x08;
    forearm_can_send_data[0] = motor1 >> 8;
    forearm_can_send_data[1] = motor1;
    forearm_can_send_data[2] = motor2 >> 8;
    forearm_can_send_data[3] = motor2;
    forearm_can_send_data[4] = motor3 >> 8;
    forearm_can_send_data[5] = motor3;
    forearm_can_send_data[6] = motor4 >> 8;
    forearm_can_send_data[7] = motor4;

    HAL_CAN_AddTxMessage(&CATCH_CAN, &forearm_tx_message, forearm_can_send_data, &send_mail_box);
}
 ...
 * @retval         电机数据指针
 */
const motor_measure_t *Can_receive::get_forearm_motive_motor_measure_point(uint8_t i)
{
    return &forearm_motive_motor[i];
}
