...
void Can_receive::get_lift_motor_measure(uint8_t num, uint8_t data[8])
{
    chassis_lift_motor[num].last_ecd = chassis_lift_motor[num].ecd;
    chassis_lift_motor[num].ecd = (uint16_t)(data[0] << 8 | data[1]);
    chassis_lift_motor[num].speed_rpm = (uint16_t)(data[2] << 8 | data[3]);
    chassis_lift_motor[num].given_current = (uint16_t)(data[4] << 8 | data[5]);
    chassis_lift_motor[num].temperate = data[6];
    if(chassis_lift_motor[num].ecd - chassis_lift_motor[num].last_ecd > 4096)
    {
        chassis_lift_motor[num].round--;
    }
    if(chassis_lift_motor[num].ecd - chassis_lift_motor[num].last_ecd < -4096)
    {
        chassis_lift_motor[num].round++;
    }
    chassis_lift_motor[num].total_angle = (chassis_lift_motor[num].round * 360.0f) / 19.0f + chassis_lift_motor[num].ecd * 360.0f / 19.0f / 8192.0f;
    chassis_lift_motor[num].angle_err = chassis_lift_motor[num].last_total_angle - chassis_lift_motor[num].total_angle;
...
void Can_receive::can_cmd_chassis_lift_motor(int16_t motor1, int16_t motor2)
{
    uint32_t send_mail_box;
    chassis_tx_message.StdId = CAN_CHASSIS_LIFT_ALL_ID;
    chassis_tx_message.IDE = CAN_ID_STD;
    chassis_tx_message.RTR = CAN_RTR_DATA;
    chassis_tx_message.DLC = 0x08;
    chassis_can_send_data[0] = motor1 >> 8;
...
const motor_measure_t *Can_receive::get_chassis_lift_motor_measure_point(uint8_t i)
{
    return &chassis_lift_motor[i];
}
